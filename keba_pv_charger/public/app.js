...
function updateStatus(snapshot) {
  ...
  const sourceEl = document.getElementById("keba-phase-source");
  if (sourceEl) {
    const src = wallbox?.reportedPhaseSource;
    if (src === null || src === undefined) {
      sourceEl.textContent = "";
    } else {
      const SOURCE_LABELS = { 0: "keine", 1: "OCPP", 2: "REST-API", 3: "Modbus TCP", 4: "UDP (erwartet)" };
      const label = SOURCE_LABELS[src] ?? `unbekannt (${src})`;
      sourceEl.textContent = `Umschalt-Quelle laut Wallbox: ${label}`;
      sourceEl.classList.toggle("diagnostic-warn", src !== 4);
    }
  }
...
const FIELDS = {
  keba: [{ name: "host", label: "IP-Adresse", type: "text", placeholder: "192.168.0.50" }],
