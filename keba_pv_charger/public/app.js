...
function updateStatus(snapshot) {
  ...
  if (sourceEl) {
    ...
    if (src === null || src === undefined) {
      ...
      const SOURCE_LABELS = { 0: "keine", 1: "OCPP", 2: "REST-API", 3: "Modbus TCP (erwartet)", 4: "UDP" };
      const label = SOURCE_LABELS[src] ?? `unbekannt (${src})`;
      sourceEl.textContent = `Umschalt-Quelle laut Wallbox: ${label}`;
      sourceEl.classList.toggle("diagnostic-warn", src !== 3);
    }
  }
...
const FIELDS = {
  keba: [
    ...
    {
      name: "registerOffset",
      label: "Register-Offset (nur bei Problemen, normal 0)",
      type: "number",
      default: 0,
    },
  ],
