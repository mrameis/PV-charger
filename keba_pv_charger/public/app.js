...
function updateFlow(snapshot) {
  ...
  const battLabel = battPowerW >= 0 ? `+${fmtW(battPowerW)} (lädt)` : `${fmtW(battPowerW)} (entlädt)`;
  document.getElementById("val-batt").textContent = batteries.length ? battLabel : "keine Batterie";
  document.getElementById("val-batt-solar").textContent = `Solar: ${fmtW(battSolarW)}`;
...
function updateSources(snapshot) {
  ...
  if (!snapshot.batteries.length) {
    ...
    for (const b of snapshot.batteries) {
      ...
      const solarTxt = b.solarPowerW !== null ? ` · Solar ${fmtW(b.solarPowerW)}` : "";
      li.innerHTML = b.online
        ? `<span>${b.name} <small>(${socTxt}${solarTxt})</small></span><span class="source-value">${fmtW(b.powerW)}</span>`
