...
## Batterie-Schutz fürs Auto-Laden

Neuer Block in den Regelparametern (Einstellungen -> Regelparameter -> „Batterie-Schutz"),
relevant für PV-Überschussmodi (Nur PV / Min+PV) sobald unter „Batterie" ein Gerät
konfiguriert ist:

- **Batterie-Schutz aktiv**: Solange eingeschaltet, wird jede aktuelle
  Batterie-Entladung genauso behandelt wie Netzbezug - die Wallbox reduziert also
  automatisch, sobald der Haushalt (unabhängig vom Auto) gerade regulär aus der
  Batterie versorgt wird. Das ist besonders bei einem Victron-Setup im
  **Zero-Feed-in-ESS-Modus** wichtig: dort hält die ESS-Regelung den Netzzähler
  bewusst bei ~0 W und deckt jedes Defizit unbemerkt aus der Batterie - ohne dieses
  Signal würde die Wallbox nie merken, dass sie gerade den Speicher mitentlädt.
- **Mindest-Ladezustand fürs Auto-Laden (%)**: Unterhalb dieses Ladezustands wird
  das Laden aus der Batterie hart pausiert (2%-Hysterese gegen Geflacker genau an
  der Schwelle). Bei mehreren konfigurierten Batterien zählt jeweils der
  niedrigste gemeldete Ladezustand.

Ohne konfigurierte Batterie hat der Schalter keine Wirkung.
