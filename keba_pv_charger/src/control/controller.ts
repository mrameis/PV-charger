...
export class PvSurplusController {
  ...
  /** Sticky-Flag mit Hysterese, siehe batteryDischargeSignal()/tick() - verhindert
   * Ein/Aus-Geflacker, wenn der Ladezustand genau um die Schwelle pendelt. */
  private batteryBlockActive = false;
  ...
  /**
   * Fasst alle konfigurierten Batterien zu einem Regelsignal zusammen:
   * - dischargeW: Summe der aktuellen Entladeleistung aller Batterien (0, falls
   *   in Summe geladen wird oder keine online ist). Wird wie zusätzlicher
   *   Netzbezug behandelt (siehe tick()), damit die Wallbox auch bei einem
   *   Zero-Feed-in-ESS reagiert, wo der Netzzähler die Entnahme nicht zeigt.
   * - minSocPercent: der niedrigste gemeldete Ladezustand aller Batterien (die
   *   konservativste Batterie entscheidet über die Schutzgrenze).
   */
  private batteryDischargeSignal(batteries: BatteryStatus[]): {
    dischargeW: number;
    minSocPercent: number | null;
  } {
    const online = batteries.filter((b) => b.online);
    if (!online.length) return { dischargeW: 0, minSocPercent: null };
    const totalPowerW = online.reduce((sum, b) => sum + (b.powerW ?? 0), 0);
    const socValues = online.map((b) => b.socPercent).filter((s): s is number => s !== null);
    return {
      dischargeW: Math.max(0, -totalPowerW),
  ...
  private async tick(): Promise<void> {
    ...
    if (!this.wallbox) {
      ...
      // Batterie-Schutz: Entladung wird wie Netzbezug behandelt (siehe
      // batteryDischargeSignal), unterhalb der Mindest-SoC-Grenze wird das Laden
      // aus der Batterie hart unterbunden - mit 2%-Hysterese gegen Geflacker genau
      // an der Schwelle.
      const { dischargeW: batteryDischargeW, minSocPercent } = this.settings.batteryProtectionEnabled
        ? this.batteryDischargeSignal(batteries)
        : { dischargeW: 0, minSocPercent: null };

      if (minSocPercent !== null) {
        if (batteryDischargeW > 0 && minSocPercent <= this.settings.minBatterySocForChargingPercent) {
          this.batteryBlockActive = true;
        } else if (minSocPercent >= this.settings.minBatterySocForChargingPercent + 2) {
          this.batteryBlockActive = false;
        }
      } else {
        this.batteryBlockActive = false;
      }
      const batteryBlocksCharging = this.settings.batteryProtectionEnabled && this.batteryBlockActive;

      // "Virtueller Netzbezug": Batterie-Entladung wird von der Überschussleistung
      // abgezogen, damit die Wallbox reduziert, sobald der Haushalt (unabhängig vom
      // Auto) gerade regulär aus der Batterie versorgt wird.
      let targetPowerW = currentChargingPowerW - gridPowerW - batteryDischargeW;
      ...
      if (blocked) {
        ...
        await this.safeCall(() => this.wallbox!.setEnabled(false));
        await this.safeCall(() => this.wallbox!.setChargingCurrentA(0));
      } else if (batteryBlocksCharging) {
        targetCurrentA = 0;
        this.wantPauseSince = null;
        note = `Batterie-Schutz: Ladezustand (${minSocPercent}%) bei/unter Grenze von ${this.settings.minBatterySocForChargingPercent}% - Laden aus der Batterie pausiert.`;
        await this.safeCall(() => this.wallbox!.setEnabled(false));
        await this.safeCall(() => this.wallbox!.setChargingCurrentA(0));
      } else {
        const minP = this.minPowerW(this.activePhases);
        ...
        if (targetPowerW < minP) {
          ...
          if (stableFor >= this.settings.decisionStabilitySec) {
            targetCurrentA = 0;
            note =
              batteryDischargeW > 50
                ? `Zu wenig Überschuss - Batterie entlädt gerade mit ${Math.round(batteryDischargeW)} W (regulärer Hausverbrauch) - Ladung pausiert.`
                : `Zu wenig PV-Überschuss (< ${Math.round(minP)} W) - Ladung pausiert.`;
          } else {
          ...
          note =
            batteryDischargeW > 50
              ? `Überschussladen aktiv, ${this.activePhases}-phasig (Batterie-Entnahme von ${Math.round(batteryDischargeW)} W wird abgezogen).`
              : `Überschussladen aktiv, ${this.activePhases}-phasig.`;
        }
