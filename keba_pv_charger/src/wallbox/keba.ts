 ...
 * (offizielles KEBA-Dokument), zusätzlich abgeglichen mit dem "UDP Programmers Guide
 * V2.04" (KeContactP30UDP_pgen.pdf), da UDP- und Modbus-Schnittstelle dieselbe
 * zugrundeliegende Funktionalität mit paralleler Semantik abbilden:
 *
 *  - UDP "x2src" == Modbus Register 5050 (Schreiben) / 1550 (Lesen der Quelle):
 *    0 = kein Umschaltkanal, 1 = OCPP, 2 = REST-API Direct, 3 = Modbus TCP, 4 = UDP.
 *    Ohne 5050=3 ignoriert die Wallbox jeden Umschaltbefehl über Modbus, selbst wenn
 *    5052 korrekt gesetzt wird. Diese App setzt 5050 vor jeder Umschaltung neu auf 3.
 *  - UDP "x2" == Modbus Register 5052 (Schreiben): 0 = 1-phasig, 1 = 3-phasig
 *    (offiziell bestätigt durch den UDP-Guide, deckt sich mit unserer Implementierung).
 *  - Laut UDP-Guide ist zwischen zwei Umschaltungen ein Cooldown von 5 Minuten
 *    vorgeschrieben (deckt sich mit unserem Standard-Cooldown von 300s).
 *  - Register 1552 (Lesen) meldet laut Doku direkt "1 oder 3 Phasen" (nicht die
 *    0/1-Schreibkonvention) - beide Fälle werden beim Lesen abgedeckt.
 *
 ...
 *  - Manche Quellen berichten, dass DSW1.2 auf OFF stehen muss, damit X2 überhaupt
 *    für die Phasenumschaltung (statt z.B. als Ladestatus-Kontakt) genutzt wird -
 *    bitte an der Wallbox selbst prüfen, das ist reine Hardware-Konfiguration.
 *  - Manche Modbus-Implementierungen zählen Registeradressen ab 1 statt ab 0 - falls
 *    Umschaltung/Status trotz korrekter Werte nicht funktionieren, in den
 *    Ladestation-Einstellungen den "Register-Offset" auf 1 (oder -1) testen.
...
export class KebaWallbox implements WallboxClient {
  ...
  async readStatus(): Promise<WallboxStatus> {
    ...
    try {
      ...
      // Laut UDP-/Modbus-Doku "enthält den Phasenumschalt-Status (1 oder 3 Phasen)" -
      // zur Sicherheit wird zusätzlich die 0/1-Schreibkonvention abgedeckt.
  ...
  async setPhases(phases: 1 | 3): Promise<void> {
    ...
    // Diagnose: sofort zurücklesen, was die Wallbox jetzt tatsächlich meldet, um
    // Off-by-one-Registeradressierung oder ignorierte Befehle sichtbar zu machen.
    try {
      const sourceAfter = await this.readUint32(REG.phaseSwitchSourceRead);
      const stateAfter = await this.readUint32(REG.phaseSwitchState);
      logger.info(
        `Keba: Phasenumschaltung ausgelöst -> ${phases}-phasig ` +
          `(Quelle danach: ${sourceAfter}, Status danach: ${stateAfter} - ` +
          `Quelle sollte 3 sein, Status ändert sich meist erst nach einigen Sekunden)`
      );
    } catch {
