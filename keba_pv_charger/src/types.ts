...
export interface ControlSettings {
  ...
  manualPowerW: number;
  /**
   * Batterie-Schutz für die PV-Überschussmodi (pv_only/min_plus_pv): wenn aktiv,
   * wird jede Entladung der Batterie wie Netzbezug behandelt (Wallbox reduziert
   * Leistung entsprechend) und unterhalb von minBatterySocForChargingPercent wird
   * das Laden aus der Batterie komplett unterbunden. Ohne konfigurierte Batterie
   * ohne Wirkung.
   */
  batteryProtectionEnabled: boolean;
  /** Mindest-Ladezustand (%) der Batterie, unterhalb dessen sie nicht fürs Auto-Laden entladen werden darf. */
  minBatterySocForChargingPercent: number;
}
