...
function rowToDevice(r: DeviceRow): DeviceConfig {
  return {
    ...
    active: !!r.active,
    enabled: !!r.enabled,
    registerOffset: r.register_offset,
  };
}
...
export class DevicesDb {
  ...
  create(d: Omit<DeviceConfig, "id">): DeviceConfig {
    ...
    const res = this.db
      .prepare(
        `INSERT INTO devices (category, device_type, name, host, port, unit_id, generation, invert, xml_path, active, enabled, register_offset)
         VALUES (@category, @deviceType, @name, @host, @port, @unitId, @generation, @invert, @xmlPath, @active, @enabled, @registerOffset)`
      )
      .run({
        category: d.category,
        ...
        registerOffset: d.registerOffset ?? null,
      });
    ...
    this.db
      .prepare(
        `UPDATE devices SET device_type=@deviceType, name=@name, host=@host, port=@port, unit_id=@unitId,
         generation=@generation, invert=@invert, xml_path=@xmlPath, enabled=@enabled, register_offset=@registerOffset WHERE id=@id`
      )
      .run({
        id,
        ...
        registerOffset: merged.registerOffset ?? null,
      });
    return this.get(id);
  }
