...
export class HistoryDb {
  ...
  getDailyTotals(days: number): DailyHistoryEntry[] {
    ...
    for (const row of rows) {
      ...
      const intervalStart = prevTs;
      const intervalEnd = row.ts;
      prevTs = row.ts;

      const dtHours = (intervalEnd - intervalStart) / 3_600_000;
      ...
      const pvShareW = estimatePvShareW(chargingPowerW, gridPowerW);

      // Intervall an Tagesgrenzen (Mitternacht) aufsplitten, statt es komplett dem
      // Tag des End-Zeitstempels zuzurechnen - sonst "verschwindet" die letzte
      // Nacht-Energie eines Tages in den Wert des Folgetags.
      let segStart = intervalStart;
      while (segStart < intervalEnd) {
        const d = new Date(segStart);
        const nextMidnight = new Date(d.getFullYear(), d.getMonth(), d.getDate() + 1).getTime();
        const segEnd = Math.min(nextMidnight, intervalEnd);
        const segHours = (segEnd - segStart) / 3_600_000;

        const dateKey = dateKeyOf(segStart);
        const entry = byDate.get(dateKey) ?? { date: dateKey, chargedWh: 0, pvWh: 0 };
        entry.chargedWh += chargingPowerW * segHours;
        entry.pvWh += pvShareW * segHours;
