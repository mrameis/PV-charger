import { VictronClient, getSharedVictronClient } from "./victron";
...
export class VictronPvSource {
  ...
  constructor(host: string, port: number, unitId: number, private label: string) {
    // Gemeinsam genutzte Verbindung (siehe victron.ts) - wichtig, falls dasselbe
    // Gerät zusätzlich als Batterie eingetragen ist.
    this.client = getSharedVictronClient(host, port, unitId);
  }
